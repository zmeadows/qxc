...
static void qxc_tokenizer_init(struct qxc_tokenizer* tokenizer,
                               const char* filepath) {
...
static bool is_valid_keyword(const char* str) {
    static const char* VALID_KEYWORDS[] = {"int", "return"};
    for (size_t i = 0; i < sizeof(VALID_KEYWORDS) / sizeof(const char*); i++) {
        if (!strcmp(str, VALID_KEYWORDS[i])) {
            return true;
        }
    }
    return false;
...
static bool is_valid_keyword_identifier_trailing_character(char c) {
    return is_valid_keyword_identifier_first_character(c) ||
           (c >= '0' && c <= '9');
}
...
static void qxc_tokenizer_consume_id(struct qxc_tokenizer* tokenizer) {
    ...
    while (
        is_valid_keyword_identifier_trailing_character(tokenizer->next_char)) {
...
struct qxc_token_buffer* qxc_tokenize(const char* filepath) {
    ...
    while (1) {
        if (is_valid_keyword_identifier_first_character(tokenizer.next_char)) {
            ...
            new_token->type = is_valid_keyword(tokenizer.id)
                                  ? qxc_keyword_token
            ...
            debug_print("encountered unexpected character: %c %d\n",
                        tokenizer.next_char, (int)tokenizer.next_char);
            break;
        }
    }
